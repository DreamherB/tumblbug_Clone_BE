...
try {
    router.patch(
        ...
        async (req, res) => {
            ...
            const thatArticle = await articles.findOne({
                articleId: articleId,
            });

            // 사용자가 중복 클릭했을 경우 false를 리턴
            if (
                thatArticle.donator.filter((e) => e.email === user.email)
                    .length > 0
            ) {
                res.json({
                    result: false,
...
try {
    router.patch(
        ...
        async (req, res) => {
            ...
            // 사용자가 이미 후원 취소 했을 경우
            if (name.donator.findIndex((e) => e.email === user.email) === -1) {
                res.json({
                    result: false,
                    msg: "이미 취소된 후원입니다.",
                });
            ...
            await articles.findOneAndUpdate(
                ...
                {
                    $inc: { totalAmount: -50000 },
                    donator: name.donator.filter((e) => e.email !== user.email),
                }
            );
